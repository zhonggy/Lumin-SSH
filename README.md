...
- **导入/导出** — 主机列表工具栏的数据管理入口支持导出全部节点（含凭据）为 **明文 JSON** 或 **密文 .enc**；密文导出可复用本机已配置的云同步密钥（零交互）或用自定义密码加密；导入时智能识别明文/密文，密文自动尝试本机云同步密钥，失败则提示输入密码；支持直接导入云端备份的 `.enc` 文件；提供导入模板下载，方便批量录入与跨机器迁移
