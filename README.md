...
- **多端一键恢复** — 在新设备上配置同步后端后即可恢复所有服务器配置
- **自动同步模式** — 启动时按偏好自动执行合并同步
...
| `snapshot_time` | 快照时间戳 |
| `ai_global_settings.json` | AI 全局设置（供应商选择、自动批准、斜杠命令等） |
| `ai_providers.json` | AI 供应商配置列表 |
...
| **网络** | 延迟检测协议选择（SSH Banner RTT / TCP Dial）、探针与延迟刷新间隔、WebView GPU 硬件加速开关 |
